...
"nlrq.control" <- function (maxiter=100, k=2, big=1e+20, eps=9.9999999999999995e-08, beta=0.96999999999999997)
{
    list(maxiter=maxiter, k=k, big=big, eps=eps, beta=beta)
}

# Still needs to be clean up: several parts of the code not used here (QR,...)
	...
	control, trace=FALSE, method = "L-BFGS-B")
{
    ...
    nlrq.calc <- function (model, ctrl, trace) {
		...
		lower=0, upper=1, Step=Step, model=model, pars=Pars, control=optim.ctrl)$par
            ...
            if (trace == TRUE) {model$trace()}
            if (R.Version()$os == "Win32") {flush.console()}
            nit <- nit + 1
        }
        model
    }
    nlrq.out <- list(m=nlrq.calc(m, ctrl, trace), data=substitute(data), call=match.call(), PACKAGE = "nlrq")
